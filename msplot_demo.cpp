...
int main()
{
    ...
    for (int i = 0; i <= 200; ++i)
    {
        ...
        altitude_array.push_back(1000 *
                                 std::sin(time));  // example altitude data
        target_altitude_array.push_back(1000);     // constant target altitude

        // Calculate velocity as the derivative of altitude
        velocity_array.push_back(1000 *
                                 std::cos(time));  // derivative of sin is cos

        // Generate sawtooth and triangle waves
        sawtooth_array.push_back(500 * sawtooth(time) +
                                 500);  // Scale to 0-1000 range
        triangle_array.push_back(500 * triangle(time) +
                                 500);  // Scale to 0-1000 range
    }
    ...
    fig.plot(time_array, target_altitude_array, "Target Altitude",
             Color(Color::Red));
    ...
    if (fig.save(filename))
    {
        ...
        // system(("open " + filename).c_str()); // Open the file in the default
        // browser
    }
    else
    {
